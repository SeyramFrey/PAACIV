...
export function FormulaireContenuLigne({
  ...
  return (
    <form
      onSubmit={soumettre}
      className="grid gap-3 rounded-xl border border-filet bg-fond/60 p-4 sm:grid-cols-[1fr_1fr_auto] sm:items-end"
    >
      <input type="hidden" name="cle" value={cle} />
      <p className="font-mono text-xs text-doux sm:col-span-3">{cle}</p>
      ...
      <label className="flex flex-col text-sm">
        ...
        />
        {aCompleterFr && <span className="mt-1 text-xs font-semibold text-danger">{t('aCompleter')}</span>}
      </label>
      ...
      <label className="flex flex-col text-sm">
        ...
        <textarea
          ...
          className="rounded-xl border border-filet bg-fond px-3 py-2"
        />
        {aCompleterEn && <span className="mt-1 text-xs font-semibold text-danger">{t('aCompleter')}</span>}
      </label>
