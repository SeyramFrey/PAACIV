    ...
    <div className="space-y-6">
      ...
      {items.length === 0 ? (
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {items.map((e) => (
              <tr key={e.id} className="border-t border-creme2">
                ...
                <td className="py-2">
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs ${
                      e.statut === 'publie' ? 'bg-vert text-sable' : 'bg-creme2 text-encre'
                    }`}
                  >
                ...
                <td className="flex justify-end gap-2 py-2">
                  <Link href={`/admin/evenements/${e.id}`} className="text-brun underline">
                    {t('editer')}
                  </Link>
