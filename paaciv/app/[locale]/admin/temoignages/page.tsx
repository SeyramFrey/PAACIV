    ...
    <div className="space-y-6">
      ...
      {items.length === 0 ? (
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {items.map((tm) => (
              <tr key={tm.id} className="border-t border-filet">
                ...
                <td className="py-2">
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs ${
                      tm.statut === 'publie' ? 'bg-vert text-sable' : 'bg-fond2 text-encre-t'
                    }`}
                  >
                ...
                <td className="flex justify-end gap-2 py-2">
                  <Link href={`/admin/temoignages/${tm.id}`} className="text-ocre underline">
                    {t('editer')}
                  </Link>
